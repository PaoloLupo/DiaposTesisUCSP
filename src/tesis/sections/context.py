...
def _text(scene : Scene, content: str, x: float, y: float, *, size: float=0.28, color: Color=BLACK, center: bool=False):
    return scene.text(
        content, size=size, text_align="center" if center else "left",
        line_spacing=1.18,
    ).fill(color).move_to(x, y, Anchor.CENTER if center else Anchor.TOP_LEFT)


def _header(scene: Scene, step: str, headline: str):
    eyebrow = _text(scene, f"PROBLEMÁTICA  /  {step}", -7.2, 3.9,
                    size=0.18, color =ACCENT)
    title = _text(scene, headline, -7.2, 3.35, size=0.51)
    rule = scene.geometry.line(-7.2, 2.6, 7.2, 2.6).stroke(ACCENT, 0.035)
    scene.play([
        eyebrow.animate.fade_in(), title.animate.write(), rule.animate.create(),
    ], duration=0.8)
...
def _arrow(scene: Scene, x1: float, y1: float, x2: float, y2: float, color: Color):
    ...
    points = [(0, 0.018), (length - head, 0.018), (length - head, 0.075),
              (length, 0), (length - head, -0.075),
              (length - head, -0.018), (0, -0.018)]
    return scene.geometry.polygon([
        (x1 + along * ux - across * uy, y1 + along * uy + across * ux)
        for along, across in points
    ]).fill(color).no_stroke()


def materials(scene: Scene):
    _ = scene.segment(
        "Problemática", Transition.cross_fade(0.55),
        notes=(
    ...
    _header(scene, "01  CONTEXTO NACIONAL",
            "*Un material extendido* en un país sísmico")

    scene.camera.save("contexto-general")
    # SVG stroke widths are local to the asset and scale with its geometry.
    outline = (scene.media.svg("peru.svg").no_fill().stroke(INK_MUTED, 2.0)
               .scale_to(0.0054).move_to(0, -0.2))
    amount = scene.viz.parameter(0.0)
    percentage = (scene.viz.readout(amount, format=".1f", suffix="%", font_size=0.8)
                  .fill(BLACK).move_to(-0.7, -0.95))
    fill = scene.geometry.fill_level(
        outline, ORANGE, 0.0, direction="up", keep_outline=False,
    ).z_index(-1)
    map_group = scene.geometry.group([outline, fill, percentage])
    caption = _text(scene, "Viviendas con paredes de\n*ladrillo o bloque de concreto*",
                    0, -2.85, size=0.27, center=True)
    scene.play(outline.animate.write(), duration=0.85)
    scene.play([
        amount.animate.set(PORCENTAJES[0]),
        fill.animate.fill_level(PORCENTAJES[0] / 100),
        percentage.animate.fade_in(), caption.animate.fade_in(),
    ], duration=1.35)
    ...
    colors = [ORANGE, "#B7791F", "#8B5E3C", "#A16207", "#64748B",
              "#D97706", "#78716C", "#94A3B8", "#475569"]
    spec = (
        ChartSpec(CHART_DATA, key="id")
        .mark("bar", width=0.72, label_position="outside", label_offset=0.18)
        .encode(
            x="material", y="viviendas_porcentaje", label="rotulo",
            color=Field("color_material", scale=Scale.category(MATERIALES).colors(colors)),
        )
        .axes(x=Axis.category(MATERIALES), y=Axis.linear(0, 70).ticks(10))
    )
    chart = scene.viz.chart(spec).scale_to(0.68).move_to(2.65, -0.45)
    chart_title = _text(scene, "Material predominante en paredes", 2.65, 1.85,
                        size=0.30, center=True)
    units = _text(scene, "Viviendas (%)", -1.5, 1.85, size=0.20, color=INK_MUTED)
    source = _text(scene, f"Fuente: {SOURCE_LABEL}", 2.65, -3.25,
                   size=0.19, color=INK_MUTED, center=True)
    scope = _text(scene, "El material de la pared no identifica el sistema estructural.",
                  0, -3.95, size=0.24, color=INK_MUTED, center=True)
    scene.play([
        chart.layer("axes").animate.create(), chart_title.animate.fade_in(),
    ...
    scene.play(stagger(
        ...
        source.animate.fade_in().duration(0.4), each=0.35,
    ))
...
def _wall_plan(scene):
    ...
    cx, cy = -3.9, -0.25
    boundary = (scene.geometry.rect(4.7, 3.35).no_fill().stroke(RULE, 0.025)
                .move_to(cx, cy))
    horizontal = []
    vertical = []
    endpoints = set()
    for x, y, width in [(-1.55, 1.55, 1.4), (1.25, 1.55, 2.0),
                        (-1.35, -1.55, 1.8), (1.575, -1.55, 1.35),
                        (-1.25, 0.0, 2.0), (1.575, 0.0, 1.35)]:
        horizontal.append(scene.geometry.rect(width, 0.13).fill(ORANGE).no_stroke()
                          .move_to(cx + x, cy + y))
        endpoints.update((round(cx + x + offset, 4), round(cy + y, 4))
                         for offset in (-width / 2, width / 2))
    for x, y, height in [(-2.25, 0.775, 1.55), (-2.25, -1.075, 0.95),
                         (2.25, 0.0, 3.1), (0.25, 0.9, 1.3),
                         (0.25, -1.05, 1.0)]:
        vertical.append(scene.geometry.rect(0.13, height).fill(ACCENT).no_stroke()
                        .move_to(cx + x, cy + y))
        endpoints.update((round(cx + x, 4), round(cy + y + offset, 4))
    ...
    return boundary, horizontal, vertical, [x_arrow, x_label], [y_arrow, y_label], sorted(endpoints)


def _distribution(scene):
    scene.segment(
        "Problemática · distribución", Transition.cross_fade(0.55),
        notes=(
    ...
    _header(scene, "02  EXIGENCIA ESTRUCTURAL",
            "La *distribución de muros* importa")
    boundary, horizontal, vertical, x_axis, y_axis, endpoints = _wall_plan(scene)
    plan_label = _text(scene, "DISTRIBUCIÓN EN PLANTA", -3.9, 1.85,
                       size=0.20, color=INK_MUTED, center=True)
    caveat = _text(scene, "Esquema conceptual · sin escala", -3.9, -3.1,
                   size=0.18, color=INK_MUTED, center=True)
    first = _text(scene, "01", 0.35, 1.6, size=0.25, color=WARM)
    first_title = _text(scene, "*Muros en ambas direcciones*", 1.05, 1.65, size=0.33)
    first_body = _text(scene, "La densidad de muros se revisa\nen X y en Y.",
                       1.05, 1.0, size=0.28, color=INK_MUTED)
    takeaway = _text(scene, "Cada cambio de distribución exige volver a verificar.",
                     0, -3.8, size=0.30, color=ACCENT, center=True)
    scene.play([boundary.animate.create(), plan_label.animate.fade_in(),
                caveat.animate.fade_in()], duration=0.55)
    scene.play(stagger(*[wall.animate.grow_from_center().duration(0.55)
                         for wall in horizontal], each=0.07))
    scene.play([item.animate.fade_in() for item in [*x_axis, first, first_title, first_body]],
               duration=0.6)
    scene.play(stagger(*[wall.animate.grow_from_center().duration(0.55)
                         for wall in vertical], each=0.07))
    ...
    for x, y in endpoints:
        columns.append(scene.geometry.rect(0.18, 0.18).fill(BLACK).no_stroke().move_to(x, y))
    scene.play(stagger(*[col.animate.grow_from_center().duration(0.4)
                         for col in columns], each=0.045))
    ...
    second_body = _text(scene, "Muros y elementos de confinamiento\ndeben trabajar en conjunto.",
                        1.05, -0.95, size=0.28, color=INK_MUTED)
    scene.play([item.animate.fade_in() for item in [second, second_title, second_body]],
               duration=0.6)
...
def _workflow_node(scene, x, number, title, body):
    outline = (scene.geometry.rounded_rect(3.8, 1.75, 0.08)
               .no_fill().stroke(RULE, 0.025).move_to(x, 0.65))
...
def _manual_workflow(scene):
    scene.segment(
        "Problemática · proceso manual", Transition.cross_fade(0.55),
        notes=(
    ...
    _header(scene, "03  PROCESO CONVENCIONAL",
            "La verificación depende de *pasos manuales*")
    nodes = [
        _workflow_node(scene, -5, "01", "Modelo en ETABS", "Geometría y resultados\ndel análisis estructural"),
        _workflow_node(scene, 0, "02", "Hojas de cálculo", "Extraer, ordenar\ny procesar datos"),
        _workflow_node(scene, 5, "03", "Verificación E.070", "Revisar requisitos\nde la distribución"),
    ]
    arrows = [_arrow(scene, x, 0.65, x + 1.05, 0.65, ORANGE)
              for x in (-3.05, 1.95)]
    transfer = [_text(scene, "trasladar", x, 1.13, size=0.18, color=WARM, center=True)
                for x in (-2.5, 2.5)]
    scene.play(nodes[0].animate.fade_in_from(Direction.DOWN, distance=0.15), duration=0.55)
    for index in range(2):
        scene.play([arrows[index].animate.create(), transfer[index].animate.fade_in()], duration=0.45)
        scene.play(nodes[index + 1].animate.fade_in_from(Direction.DOWN, distance=0.15), duration=0.55)
    scene.stop("flujo-manual")

    risks = [_text(scene, text, x, -0.95, size=0.25, color=WARM, center=True)
             for x, text in [(-5, "Criterios de modelado"), (0, "Transcripción de datos"),
                             (5, "Verificaciones omitidas")]]
    scene.play(stagger(*[risk.animate.fade_in().duration(0.45) for risk in risks], each=0.2))
    return_path = (scene.geometry.polyline([(5, -1.45), (5, -2.0), (-7.1, -2.0),
                                           (-7.1, 0.65), (-6.98, 0.65)])
                   .no_fill().stroke(ACCENT, 0.035))
    arrow_tip = _arrow(scene, -7.1, 0.65, -6.91, 0.65, ACCENT)
    repeat = _text(scene, "Modificar la distribución → repetir la revisión", 0, -2.35,
                   size=0.24, color=ACCENT, center=True)
    ...
    question = _text(scene, "¿Cómo *sistematizar* la verificación de la distribución de muros?",
                     0, -3.45, size=0.34, center=True)
